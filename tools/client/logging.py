...
class HarvestTraceback(Traceback):
    # DEP-WARN
    @group()
...
class HarvestLogRender(LogRender):
    def __call__(
        self,
        console: Console,
...
class HarvestRichHandler(RichHandler):
    ...
    def emit(self, record: LogRecord) -> None:
        ...
        if (
            ...
            traceback = HarvestTraceback.from_exception(
                exc_type,
                exc_value,
                exc_traceback,
...
def init_logging(level: int) -> None:
    ...
    stdout_handler = HarvestRichHandler(
